			...
			<div class="sidebox">			
				...
				<ul class="sidemenu">
					...
					<%
						...
						List<Insumo> varInsumo = qInsumo.getResultList();
					%>
...
<div id="main">				
	<div class="post">
		...
		<ul>	
		<p>Lista de todas os insumos cadastrados</p>	
		<form id="milho" action="../ServletInsumo" method="get">																											
			<table>
							<tr>
								<th class="first"><strong>Nome item</strong></th>
								<th><p class="buttonSalva"><a title="Cadastrar" href="insumo.jsp"></a></p></th>
							</tr>
								<%
									int a = 0;
									for (Insumo chamaInsumo : varInsumo) {
								%>
									<tr class="row-<%=(a % 2 == 0) ? "a" : "b" %>">
										<td class="first"><%=chamaInsumo.getNomeInsumo()%></td>
										<td></td>
										<td><p class="buttonAlt"><a title="Alterar" href="alterainsumo.jsp?idInsumo=<%=KeyFactory.keyToString(chamaInsumo.getId()) %>"></a></td>
										<td><p class="buttonDel"><a title="Apagar" href="../ServletInsumo?acao=Remover&idInsumo=<%= KeyFactory.keyToString(chamaInsumo.getId())%>"></a></td>
									</tr>
								<%
										a++;
									}
