...
import { Manejo } from '../../core/models';
  ...
  template: `
    <div class="p-8">
      ...
      <div class="flex items-center justify-between mb-8">
        <div>
          <h2 class="text-3xl font-bold text-on-surface font-headline">Manejo</h2>
          <p class="text-on-surface-variant mt-1 text-sm">Gerencie os tipos de manejo</p>
        </div>
        <button
          (click)="openCreate()"
      ...
      <div class="bg-surface-container-lowest rounded-xl overflow-hidden">
        @if (loading()) {
          ...
          <div class="flex flex-col items-center justify-center py-16 text-on-surface-variant">
            <span class="material-symbols-outlined text-5xl mb-3 opacity-30">manage_accounts</span>
            <p class="text-sm">Nenhum tipo de manejo cadastrado</p>
            <button
              (click)="openCreate()"
        ...
        <div class="relative bg-surface-container-lowest rounded-xl p-6 w-full max-w-md shadow-2xl">
          <div class="flex items-center justify-between mb-5">
            <h3 class="text-lg font-semibold text-on-surface font-headline">
              {{ editItem() ? 'Editar Manejo' : 'Novo Manejo' }}
            </h3>
            <button
              (click)="closeModal()"
          ...
          <form [formGroup]="form" (ngSubmit)="onSave()" class="space-y-4">
            <div>
              <label
                ...
                >Nome do Manejo</label
              >
              <input
                formControlName="nomeManejo"
                type="text"
                placeholder="Ex: Confinamento, Semi-confinamento..."
              ...
              />
              @if (form.get('nomeManejo')?.invalid && form.get('nomeManejo')?.touched) {
...
export class ManejoComponent implements OnInit {
  ...
  readonly items = signal<Manejo[]>([]);
  readonly loading = signal(false);
  readonly savingItem = signal(false);
  readonly showModal = signal(false);
  readonly editItem = signal<Manejo | null>(null);
  readonly error = signal<string | null>(null);
  readonly toast = signal<string | null>(null);

  readonly form = this.fb.group({
    nomeManejo: ['', Validators.required],
  });
  ...
  loadItems() {
    ...
    this.error.set(null);
    this.manejoService.listar().subscribe({
      next: (data) => {
        this.items.set(data);
        this.loading.set(false);
      },
      error: () => {
        this.error.set('Erro ao carregar manejos.');
        this.loading.set(false);
      },
    });
  }
  ...
  openEdit(item: Manejo) {
    this.editItem.set(item);
    this.form.patchValue({ nomeManejo: item.nomeManejo });
    this.showModal.set(true);
  }
  ...
  onSave() {
    ...
    const payload: Partial<Manejo> = {
      nomeManejo: this.form.value.nomeManejo!,
    };

    const item = this.editItem();
    const op = item?.id
      ? this.manejoService.atualizar(item.id, payload)
      : this.manejoService.criar(payload);
    ...
    op.subscribe({
      next: () => {
        ...
        this.closeModal();
        this.loadItems();
        this.showToast(item?.id ? 'Manejo atualizado!' : 'Manejo cadastrado!');
      },
      error: () => {
  ...
  onDelete(id: number) {
    if (!confirm('Deseja remover este manejo?')) return;
    this.manejoService.remover(id).subscribe({
      next: () => {
        this.loadItems();
        this.showToast('Manejo removido!');
      },
